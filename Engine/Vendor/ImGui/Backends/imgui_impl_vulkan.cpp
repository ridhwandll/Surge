...
struct ImGui_ImplVulkan_Data
{
    ...
    VkDescriptorPool            InternalDescriptorPool; // For fonts 
    VkPipeline                  Pipeline;
    uint32_t                    Subpass;
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                // Bind descriptor set with the font_texture or the texture inputed by the user
                VkPipelineLayout pipelineLayout = bd->PipelineLayout;
...
bool ImGui_ImplVulkan_CreateFontsTexture(VkCommandBuffer command_buffer)
{
    ...
    // Create/Update the Descriptor Set:
    VkDescriptorSet font_descriptor_set;
    {
        // Allocate the descriptor set into the internal descriptor pool (responsible only for fonts)
        ...
        check_vk_result(vkAllocateDescriptorSets(v->Device, &alloc_info, &font_descriptor_set));
...
bool    ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info, VkRenderPass render_pass)
{
    ...
    ImGui_ImplVulkan_Init_InternalDescriptors();
...
IMGUI_IMPL_API void ImGui_ImplVulkan_Init_InternalDescriptors()
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
    ...
    check_vk_result(vkCreateDescriptorPool(v->Device, &pool_info, nullptr, &bd->InternalDescriptorPool));
}
...
void ImGui_ImplVulkan_Shutdown()
{
    ...
    // Destroy the internal descriptor pool
    vkDestroyDescriptorPool(v->Device, bd->InternalDescriptorPool, nullptr);
...
IMGUI_IMPL_API ImTextureID ImGui_ImplVulkan_AddTexture(VkImageView image_view, VkImageLayout image_layout, VkSampler sampler, VkDescriptorSet descriptor_set)
{
